...
class SharedChangeNotifier extends ChangeNotifier {
  final MutableLiveData<String> _sharedNameUser = MutableLiveData("");
  MutableLiveData<String> get sharedNameUser => _sharedNameUser;
  ...
  void updateNameUser(String newNameUser) {
    _sharedNameUser.value = newNameUser;
  }
  ...
  final MutableLiveData<String> _sharedPathPrint = MutableLiveData("");
  MutableLiveData<String> get sharedPathPrint => _sharedPathPrint;

  final MutableLiveData<bool> _sharedIsColorToPrint = MutableLiveData(false);
  MutableLiveData<bool> get sharedIsColorToPrint => _sharedIsColorToPrint;

  final MutableLiveData<bool> _sharedIsFormatoCardToPrint =
      MutableLiveData(false);
  MutableLiveData<bool> get sharedIsFormatoCardToPrint =>
      _sharedIsFormatoCardToPrint;

  void updatePathPrint(String newPath) {
    _sharedPathPrint.value = newPath;
    print("PATHHEREEEEEEEEEEEEEEEEEEEEEEEE");
    print(_sharedPathPrint.value);
  }

  void updateIsColorToPrint(bool value) {
    _sharedIsColorToPrint.value = value;
  }

  void updateIsFormatoCardToPrint(bool value) {
    _sharedIsFormatoCardToPrint.value = value;
  }

  // Future<void> handleSendEmail() async {
  //   // Elige destinatario según tu flag isColorToPrint
  //   final recipients = _sharedIsColorToPrint.value
  //       ? ['Ap0922285@gmail.com']
  //       : ['Adrianpadilala@gmail.com'];

  //   // Construye el email
  //   final Email email = Email(
  //     body: 'Archivo solicitado.',
  //     subject:
  //         'Formato: ${_sharedIsFormatoCardToPrint.value ? "Carta" : "Oficio"}',
  //     recipients: recipients,
  //     cc: [],
  //     bcc: [],
  //     attachmentPaths: [_sharedPathPrint.value], // aquí le pasas el path
  //     isHTML: false,
  //   );

  //   try {
  //     await FlutterEmailSender.send(email);
  //     print("ENVIADOOOOOOOOOOOOO");
  //     _sharedIsColorToPrint.value = false;
  //     _sharedIsFormatoCardToPrint.value = false;
  //     debugPrint('Email enviado a: $recipients');
  //   } catch (e) {
  //     _sharedIsColorToPrint.value = false;
  //     _sharedIsFormatoCardToPrint.value = false;
  //     print("NOOOOOOOOO ENVIADOOOOOOOOOOOOO");
  //     debugPrint('Error al enviar email: $e');
  //   }
  // }

  Future<void> handleSendEmail() async {
    // Elige destinatario según tu flag
    final String recipient = _sharedIsColorToPrint.value
        ? 'ap0922285@gmail.com'
        : 'adrianpadilala@gmail.com';

    // Credenciales SMTP (App Password de 16 caracteres)
    // const String username = 'mark.jorge.2001@gmail.com';
    // const String password = 'rvmd twmk mmpi khmj';
    const String username = 'printfastoficial@gmail.com';
    const String password = 'fwdc koji umiz ojdw';

    // Configura el servidor SMTP de Gmail
    final smtpServer = gmail(username, password);

    // Construye el mensaje con adjunto
    final message = Message()
      ..from = Address(username, 'PrintFastOifical')
      ..recipients.add(recipient)
      ..subject = '${_sharedMatricula.value} - ${_sharedNameUser.value}'
      ..text =
          'Formato: ${_sharedIsFormatoCardToPrint.value ? "Carta" : "Oficio"}'
      ..attachments = [FileAttachment(File(_sharedPathPrint.value))];

    try {
      // Envía el email silenciosamente
      final sendReport = await send(message, smtpServer);
      print('ENVIADOOOOOOOOOOOOO: $sendReport');
