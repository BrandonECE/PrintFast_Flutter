...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  ...
  HomeBloc({required this.authRepository}) : super(HomeInitial()) {
    on<HomeChangeIndexBottomNavigationBarEvent>((event, emit) {
      emit(state.copyWith(currentIndex: event.currentInde));
    });
  ...
  Future<void> signOut() async {
    ...
    try {
      ...
      add(
        HomeUpdateHomeStatusEvent(
          homeStatus: HomeStatus.success, messageError: null
        ),
      );
      add(HomeChangeIndexBottomNavigationBarEvent(currentInde: 0));
    } catch (e) {
      add(
        HomeUpdateHomeStatusEvent(
