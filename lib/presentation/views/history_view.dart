...
class MyHistoryView extends StatelessWidget {
  ...
  // ---------- Estatus visual (edítalos aquí para probar) ----------
  final bool _isLoading = false;
  // Para simular "vacío" pon la lista en [].
  final List<_DemoOrder> _orders = const [
    _DemoOrder("Sucursal Central", "2025-08-12", "75.00"),
    _DemoOrder("Sucursal Norte", "2025-07-29", "120.00"),
  ];
  ...
  Widget _buildTitleRow(BuildContext context, double width) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Text("Ordenes", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: inverse)),
  ...
  Widget _chooseContent(BuildContext context) {
    ...
    if (_orders.isEmpty) return _buildEmpty(context);
        ...
        return Column(
          children: [
            ...
            Container(
              ...
              child: Text(item.dateTimeComplete, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
            ),
                ...
                children: [
                  Row(
                    children: [
                      SizedBox(
                        width: 60,
                        child: Text(item.place, style: TextStyle(fontWeight: FontWeight.bold, color: inverse, overflow: TextOverflow.ellipsis))),
                      const SizedBox(width: 6),
                      Icon(Icons.location_on_rounded, color: inverse, size: 18),
                    ],
                  ),
                  Text("${item.price}\$", style: TextStyle(fontWeight: FontWeight.bold, color: inverse)),
                  ElevatedButton(
                    onPressed: () => context.push(Routes.historyOrder),
